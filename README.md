...
问题：
1. 不知道怎么实现 1.5 倍行距，所以是打开预览和 word 文档缩放至肉眼可见相同大小调节的，和 word 略微有些不同
2. 最后改：黑体字体和 word 内不同
3. todo：三线表格式、图片格式、公式格式、图表
4. todo：参考文献
5. todo：正文段落采用 1.35 倍行间距，段前、段后均为 0 行。标题采用单倍行距，按照标题的不同，采用不同的段前、段后间距：
标题级别  段前段后间距  行间距
一级标题  1.5 行        1.5 倍
二级标题  1 行          1.5 倍
三级标题  0.5 行        1.5 倍

6. todo：页眉 1.5cm，页脚 1.75cm，页眉统一为“河北工程大学毕业设计”或“河北工程大学毕业论文”。
